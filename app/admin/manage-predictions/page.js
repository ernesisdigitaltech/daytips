              ...
              {confirmDeleteId === fx.id ? (
                <div style={styles.confirmDeleteBox}>
                  <p style={styles.confirmDeleteText}>
                    Permanently delete <strong>{fx.home_team} vs {fx.away_team}</strong>? It will disappear from
                    the homepage immediately for all users. This cannot be undone.
                  </p>
                  <div style={styles.confirmActionRow}>
                    <button
                      disabled={deletingId === fx.id}
                      onClick={() => handleDeleteFixture(fx.id)}
                      style={styles.confirmDeleteBtn}
                    >
                      {deletingId === fx.id ? 'Deleting…' : 'Yes, delete permanently'}
                    </button>
                    <button
                      disabled={deletingId === fx.id}
...
const styles = {
  ...
  dangerZone: { marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(166,58,46,0.2)' },
